...
router = APIRouter(
    prefix="/classification-assignments", tags=["Classification Assignments"]
)


@router.get(
    "",
    response_model=schemas.Page[schemas.ClassificationAssignmentRead],
    summary="List classification assignments",
)
...
def create_assignment(
    data: schemas.ClassificationAssignmentCreate, db: Session = Depends(get_db)
):
    try:
...
@router.put(
    "/{assignment_id}",
    response_model=schemas.ClassificationAssignmentRead,
    summary="Update a classification assignment",
)
def update_assignment(
    assignment_id: str,
    data: schemas.ClassificationAssignmentUpdate,
    db: Session = Depends(get_db),
):
    try:
...
@router.delete(
    "/{assignment_id}", status_code=204, summary="Delete a classification assignment"
)
